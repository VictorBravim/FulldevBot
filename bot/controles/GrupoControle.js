...
async filtroAntiLink(c, mensagemBaileys, botInfo) {
    try {
        ...
        // Verifica se a mensagem é de um grupo e se o recurso Anti-Link está ativado
        ...
        if (!bot_admin) {
            ...
            // Verifica se o texto da mensagem contém links relacionados ao WhatsApp
            if (usuarioTexto) {
                const regexWhatsAppLinks = /(https?:\/\/)?(chat\.whatsapp\.com|wa\.me)\/[a-zA-Z0-9]+/g;
                const textoComWhatsAppLink = usuarioTexto.match(regexWhatsAppLinks);

                if (textoComWhatsAppLink && !admins.includes(remetente)) {
                    // Envia mensagem de aviso e deleta a mensagem contendo o link
                    await socket.enviarTextoComMencoes(
                        c,
                        id_chat,
                        criarTexto(comandos_info.grupo.alink.msgs.detectou, remetente.replace("@s.whatsapp.net", "")),
                        [remetente]
                    );
